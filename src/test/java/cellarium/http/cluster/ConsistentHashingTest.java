...
//        ConsistentHashing.createVirtualNodes("http://localhost:0", null, 10);
    }

//    @Test(expected = IllegalArgumentException.class)
    public void testEmptyClusterUrlsArgumentCreation() {
//        ConsistentHashing.createVirtualNodes("http://localhost:0", Collections.emptySet(), 10);
    }

//    @Test(expected = IllegalArgumentException.class)
    public void testZeroVirtualNodesCountArgumentCreation() {
//        ConsistentHashing.createVirtualNodes("http://localhost:0", Set.of("http://localhost:0"), 0);
    }
...
//        final Node[] virtualNodes = ConsistentHashing.createVirtualNodes("http://localhost:0", clusterUrls, virtualNodePerRealNodeAmount);
//
//        final int step = 100;
//        for (int hash = Integer.MIN_VALUE; true; hash += step) {
//            final int nodeIndexForHash = ConsistentHashing.getNodeIndexForHash(hash, virtualNodes.length);
...
//        }
//
//        final int nodeIndexForHash = ConsistentHashing.getNodeIndexForHash(Integer.MAX_VALUE, virtualNodes.length);
//        Assert.assertTrue(nodeIndexForHash < virtualNodes.length);
//        Assert.assertTrue(nodeIndexForHash >= 0);
    }
